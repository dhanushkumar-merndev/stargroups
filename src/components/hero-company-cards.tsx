...
function RandomDrift({ children, delay }: { children: ReactNode; delay: number }) {
        ...
        return (
          <motion.div
            ...
            initial={{ 
              opacity: 0, 
              scale: 0, 
              x: detail.side === "left" ? `calc(50vw - ${parseFloat(detail.offset)}vw - 50%)` : `calc(-50vw + ${parseFloat(detail.offset)}vw + 50%)`,
            ...
            style={detail.side === "left" ? { left: detail.offset, top: detail.top } : { right: detail.offset, top: detail.top }}
            className="pointer-events-auto absolute w-[clamp(11rem,16vw,18rem)]"
          >
            <RandomDrift delay={detail.delay}>
