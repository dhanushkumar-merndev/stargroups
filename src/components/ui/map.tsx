...
type MapProps = {
  ...
  /** Hide label layers from the underlying basemap while retaining geography. */
  hideLabels?: boolean;
  /** Rotate the map continuously at this many degrees per second. */
...
const Map = forwardRef<MapRef, MapProps>(function Map(
  ...
  // Carto's base style uses symbol layers for country, city, and road labels.
  // Hide those only when requested; markers and their React labels stay intact.
  useEffect(() => {
    if (!mapInstance || !isStyleLoaded || !hideLabels) return;
  ...
  useEffect(() => {
    if (!mapInstance || !isStyleLoaded || !autoRotate) return;
    ...
    const rotate = (time: number) => {
      ...
      previousTime = time;
      mapInstance.setBearing(mapInstance.getBearing() + autoRotate * elapsedSeconds);
      frameId = requestAnimationFrame(rotate);
    };

    frameId = requestAnimationFrame(rotate);
    return () => cancelAnimationFrame(frameId);
  }, [mapInstance, isStyleLoaded, autoRotate]);
